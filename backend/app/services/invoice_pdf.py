"""Renders recurring-invoice records as downloadable PDF documents - ported
from teammate feature-annapoorna's branch. Uses `reportlab`, already a
dependency here (see `lease_agreement_pdf.py`).
"""
...
def build_invoice_statement_pdf(invoices, filters: dict) -> bytes:
    ...
    rows = [["Resident", "Billing Period", "Unit", "Amount", "Due Date", "Status"]]
    total = Decimal("0")
    for invoice in invoices:
        resident_name = invoice.lease.guest.name if invoice.lease and invoice.lease.guest else "-"
    ...
    table = Table(rows, colWidths=[90, 130, 50, 80, 80, 70])
    table.setStyle(_table_style())
    elements.append(table)
