"""
HTTP layer for Invoices - ported from teammate feature-lavanya's branch,
then extended with the approve-then-pay workflow, AI insight, and PDF export
endpoints ported from teammate feature-annapoorna's branch. Adapted to this
branch's `core.security` (get_current_guest_id/require_staff/CurrentUser)
instead of either branch's own `core.deps`/`schemas.auth.CurrentUser`. No
business logic here - see app/services/invoice_service.py.

Route order matters: /insights and /export/pdf must be declared before the
dynamic /{invoice_id} routes or FastAPI would try to parse "insights"/
"export" as a UUID.
"""
...
from app.core.security import CurrentUser, get_current_guest_id, require_staff
...
from app.services import invoice_pdf, invoice_service
...
def _error(status_code: int, message: str, code: str) -> HTTPException:
    return HTTPException(status_code, detail={"message": message, "error_code": code})
...
def list_invoices(
    ...
    return SuccessResponse(
        data=[InvoiceResponse.from_model(item) for item in items],
...
def generate_invoice(
    ...
    return SuccessResponse(
        ...
        message=None if created else "Existing invoice returned",
    )
...
def get_invoice(
    ...
    return SuccessResponse(data=InvoiceResponse.from_model(invoice))
...
def submit_payment(
    ...
    try:
        ...
        raise _error(400, str(exc), "INVALID_PAYMENT_STATUS")
    return SuccessResponse(data=InvoiceResponse.from_model(invoice), message="Payment submitted for staff approval")
